  ...
  useEffect(() => {
    ...
    return (
        <Box {...props}>
          ...
          />
            <Typography variant="h6" sx={{mb: 2}}>{t("main:files.share.shared_list")}</Typography>
          {!!isSharedLoading ? (
            <Box sx={{ px: 1 }}>
              <Skeleton
                sx={{
            ...
            <Box sx={{ px: 1, flex: 1, overflowY: "auto" }}>
                ...
                {shared?.length > 0 ? shared?.map((item, itemIndex) => (
                  <Card
                    key={itemIndex}
                    type="clipboard"
                    ...
                    data={{
                      ...
                      copyContent: localStorage.getItem("hostname")+"/api/share/"+item.download_code,
                  ...
                  >
                    <Trans i18nKey={"main:share.no_results"} />
                  </Typography>
                )}
              {total > 26 && (
                <Box
